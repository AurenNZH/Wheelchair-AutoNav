...
`nav2_turn_cost_stop`, plus the transient source-age faults `stale_source` and
`stale_left_source`, automatically recover while the joystick remains held:
...
count. Missing, invalid, semantic, emergency, and other STOP reasons still
latch until joystick release. Pass
`--require-release-after-obstacle-stop` to restore release-required behavior
for all automatically recoverable STOPs. Keyboard shared control is unchanged.
